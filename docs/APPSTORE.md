...
- **`.deb`**（CPack）：`port/dist/ssh_term_0.2.2-m5stack1_arm64.deb`。APPLaunch レイアウト
  （`bin/ssh_term`＋`applications/ssh_term.desktop`＋`share/images/ssh_term.png`）、Maintainer=`u44e@users.noreply.github.com`、
  依存 `libvterm0/openssh-client/telnet/picocom` 等。→ **前述 CI の機械的関門を満たす**。
- **残る機能ギャップ（③・実機必須）**：evdev キーボードのタグ再現（Ctrl/Alt/Fn/F系/Meta）。`port/main.c` に TODO。
  現状の device ビルドは基本キーは通るが、修飾キー系は CardputerZero のキーマップが要る（実機で確定）。
