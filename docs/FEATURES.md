...
| **スクロールバック内検索**（Session Menu →「Find」：文字列入力→一致へ遡り表示、`n`で次、機器プロンプト非依存） | ✅ |
...
| 一覧（選択→接続）／**編集画面 CRUD**（新規`n`/編集`e`/**複製`c`**/削除`d`＋確認ダイアログ）／**並べ替え`[` `]`**／インライン文字入力 | ✅ |
...
| ログ一覧（mtime降順）＋閲覧（**ANSI除去**・スクロール・**検索 `/`＋次 `n`**）・削除 | ✅ |
| 実行中ログON/OFF（メニュー Toggle log） | ✅ |
| **ログのサイズ上限**（既定8MiB/セッション、`TERM_LOGCAP`で変更・0で無制限）→ 超過で切り捨て注記 | ✅ |
...
（ジャンプ `-J`・ポートフォワード `-L/-R/-D`）／複数セッション／scp/sftp 転送。
（済：鍵`-i`／ログサイズ上限／プロファイル複製／ログ内検索／並べ替え／カラーテーマ／スクロールバック検索）
