...
| 一覧（選択→接続）／**編集画面 CRUD**（新規`n`/編集`e`/**複製`c`**/削除`d`＋確認ダイアログ）／インライン文字入力 | ✅ |
...
- GitHub: `github.com/u44e/cardputerzero-ssh-term`（private、`v0.2.0`）。
...
（ジャンプ `-J`・ポートフォワード `-L/-R/-D`）／スクロールバック内の検索／プロファイル並べ替え／
カラーテーマ／複数セッション。（済：鍵`-i`／ログサイズ上限／プロファイル複製／ログ内検索）
