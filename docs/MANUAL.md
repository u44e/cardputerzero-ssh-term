...
| Find... | **スクロールバック内を検索**（文字列→`Enter`で一致へ遡り表示、`n`で次、`ESC`で最新へ） |
...
- **Pace**（`←→`で切替）：
  - `fixed 10ms/line` … 一定間隔で1行ずつ送出。
  - `wait-for-prompt` … **1行送るごとに端末出力が静まるのを待って**次行を送ります（機器が遅い・
    行ごとに応答を返す CLI で取りこぼしを防ぐ。プロンプト文字列に依存しない待ち方式）。
- `Enter`で送出開始（網機器のCLIへ設定を流す、スクリプトを貼る 等）。`ESC`で取消。
- 送出中は端末下部のステータスバー右に **`SEND nn%`**（琥珀色）で進捗が出ます。
...
`Enter`で閲覧（ANSI除去・`↑↓`スクロール、`d`削除、`ESC`戻る）。閲覧中は **`/` で検索**（文字列を入力→`Enter`で
最初の一致へジャンプ）、**`n` で次の一致**（末尾で先頭へ回り込み）：
